                ...
                <form method="post" action="{{route('save-car-settings',['key'=>$key])}}" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            ...
                            {{csrf_field()}}
                                @if($key === "model")
                                    @php $brands = App\Brand::orderby('name')->get(); @endphp
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="card-body">
                                                        ...
                                                        <div class="col-md-4">
                                                            @if(!empty($brands) && count($brands) > 0)
                                                                <select class="form-control" name="brand" required>
                                                                    ...
                                                                    @foreach($brands as $b)
                                                                        <option value="{{$b->id}}" @if(!empty($brand->brand_id) && $brand->brand_id === $b->id) selected @endif>{{$b->name}}</option>
                                                                    @endforeach
                                                                </select>
                                                            @endif
                                                        </div>
                                ...
                                @endif
                                @if($key === "city")
                                    @php $brands = App\CarState::orderby('name')->get(); @endphp
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="card-body">
                                                <div class="form-group">
                                                        ...
                                                        <div class="col-md-4">
                                                            @if(!empty($brands) && count($brands) > 0)
                                                                <select class="form-control" name="state" required>
                                                                    <option value="">Select Brand</option>
                                                                    @foreach($brands as $b)
                                                                        <option value="{{$b->id}}" @if(!empty($brand->state) && $brand->state === $b->id) selected @endif>{{$b->name}}</option>
                                                                    @endforeach
                                                                </select>
                                                            @endif
                                                        </div>
                                ...
                                @if($key === 'car-type')
                                    <div class="bg-white pt-4 px-4 pb-0 my-2 mb-4 rounded border">
                                        <div style='max-width:55px;' class='float-right m-2'>
                                            <img  id="blah" style="width: 55px;height: 25.313px" src="/../../public/car_icon/{{$brand->icon}}">
                                        </div>
                                        <h4>Icon</h4>
