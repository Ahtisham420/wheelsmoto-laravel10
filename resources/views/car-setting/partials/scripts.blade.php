<script>
    ...
    function queryString(key, dataString, string = ''){
        var _qString = "";
        if(string === ''){
            _qString = "?key=filters&category={{$key}}&"+key + "=" +dataString;
        }else{
    ...
    $(document).on('click','.js-filter-export' ,function() {
        ...
        window.open(baseurl + '/car-settings/all/{{$key}}'+_string, '_blank');
    });
    ...
    $(document).on('click','.page-link' ,function(event) {
        ...
        @if(!isset($_GET['key']))
            _url = _url  + "&key=paginate&category={{$key}}";
        @endif
        axios.get(_url)
            .then(function(response) {
                $(".js-index-table").html(response.data);
