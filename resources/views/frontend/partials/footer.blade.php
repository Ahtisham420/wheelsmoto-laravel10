...
<div class="footer">
    <div class="footer-inner">
        <div class="frame-group d-flex align-items-start flex-wrap">
            <!-- <div class="conatiner">
                <div class="row footer1">
                    <div class="col col-sm-6 col-md-6 col-lg-3 widget1">
            ...
            <div class="support-parent">
                <div class="icon-group mb-3 sub-support-parent">
                    <div class="support mb-sm-3">WheelsMoto.com</div>
                    <div class="home mb-sm-3"><a href="{{route('frontend-blog')}}">Blog</a></div>
                    <div class="home mb-sm-3"><a href="{{route('event-frontend')}}">Events</a></div>
</div>
                <div class="icon-group">
                    <div class="support mb-sm-3">Follow Us</div>
                        <div class="home">
                            <a href="https://twitter.com/wheels_hunt" target="_blank"><i class="footer-icons fa-brands fa-square-twitter" style="color:#03a9f4"></i></a>
                            <a href="https://www.instagram.com/wheels_hunt/" target="_blank"><i class="footer-icons fa-brands fa-square-instagram instagram"></i></a>
                            <a href="https://web.facebook.com/Wheels-Hunt-113432407234573?_rdc=1&_rdr" target="_blank"><i class="footer-icons fa-brands fa-facebook"  style="color:#3b5998"></i></a>
                            <a href="https://www.pinterest.co.uk/powerperfor1032/?skipFac=1" target="_blank"><i class="footer-icons fa-brands fa-pinterest"  style="color:#d50012"></i></a>
                            <a href="https://www.youtube.com/channel/UCALG4ZeuaiYESwVu_uuox5Q?view_as=subscriber" target="_blank"><i class="footer-icons fa-brands fa-youtube"  style="color:#f00"></i></a>
                        </div>
                    </div>
                </div>
            <div class="support-parent mb-support">
                <div class="support">Cars by Price</div>
                <div class="home"><a href="{{route('car-budget',['type'=>'10'])}}">Cars Under 10 Lacs</a></div>
                <div class="home"><a href="{{route('car-budget',['type'=>'20'])}}">Cars Under 20 Lacs</a></div>
                <div class="home"><a href="{{route('car-budget',['type'=>'30'])}}">Cars Under 30 Lacs</a></div>
                <div class="home"><a href="{{route('car-budget',['type'=>'40'])}}">Cars Under 40 Lacs</a></div>
                <div class="home"><a href="{{route('car-budget',['type'=>'50'])}}">Cars Under 50 Lacs</a></div>
                <div class="home"><a href="{{route('car-budget',['type'=>'60'])}}">Cars Under 60 Lacs</a></div>
            </div>
            <div class="support-parent mb-support">
                <div class="support">Cars by Category</div>
                <div class="home"><a href="{{route('index-car-type',['type'=>'sedan'])}}">Sedan</a></div>
                <div class="home"><a href="{{route('index-car-type',['type'=>'hatch-back'])}}">Hatch Back</a></div>
                <div class="home"><a href="{{route('index-car-type',['type'=>'jeep'])}}">Jeep</a></div>
                <div class="home"><a href="{{route('index-car-type',['type'=>'compact-sedan'])}}">Compact Sedan</a></div>
                <div class="home"><a href="{{route('index-car-type',['type'=>'SUV'])}}">SUV</a></div>
                <div class="home"><a href="{{route('index-car-type',['type'=>'4X4'])}}">4X4</a></div>
            </div>
            <div class="support-parent">
                <div class="newsletter">
                    <div class="newsletter-cont">
                    <form class="subscription" method="post" action="{{route('footer-mail')}}" id="footer-mail">
                            @csrf
                        <input type="hidden" name="submit" value="submit">
                            <input class="add-email form-control" id="footer-mail-input" type="email" name="mail" placeholder="subscribe@me.now" required="" >
                            <button class="submit-email" type="submit">
                                <span class="before-submit">Subscribe</span>
                                <span class="after-submit">Thank you for subscribing!</span>
                                <span class="already-submit">You are already subscribed!</span>
                            </button>
                        </form>
                    </div>
                </div>
...
<div class="band">
    <div class="band-inner">
        <div class="localhost-inc-all-rights-res-parent">
            <div class="localhost-inc-all">
                © 2024 Localhost, Inc. All Rights Reserved
            </div>
            <div class="privacy-policy-parent">
                <div class="localhost-inc-all">Privacy Policy</div>
                <div class="localhost-inc-all">Terms & Conditions</div>
                <div class="localhost-inc-all">Contact us</div>
            </div>
        </div>
    </div>
</div>
        ...
        $.ajax({
            ...
            success:function(data){
                if(data.status==1){
                    ...
                    $('.after-submit').hide();
                    $('.before-submit').hide();
                    document.querySelector('.subscription').classList.add('already-done');
                    document.querySelector('.done').css("backgroundColor", "red");
                }else{
                }
                setTimeout(function () {
    ...
    $(document).on("submit",".login_f",function(e){
        ...
        $.ajax({
            ...
            error:function(data){
                ...
                $("#login-email").css('border-color', '#e4173e');
                $("#login-password").css('border-color', '#e4173e');
                $("#login-form-validation").html(data.errors);
                $("#login-form-validation").show();
                $("#login-form-validation").css('color', '#e4173e');
                $(".red-border-input").css('border', '1px solid #e4173e');
                setTimeout(function(){
...
// this for swap
        ...
        $(document).on("change",".validate1",function(){
            $(this).next().find('.select2-selection--single').removeClass('error_select2');
        });
        $("#user_car_s").submit(function(e){
            ...
            $(".validate1").each(function(e) {
                if ($(this).val() === "") {
                    flag = false;
                    $(this).next().find('.select2-selection--single').addClass('error_select2');
                }
            });
            if (!flag){
...
//$("#bhp").val(data.result['GetVehicleDataResult']['VehicleRegistration']['EngineCapacity']);
    ...
    $(document).ready(function() {
        $('#listview').click(function(event){event.preventDefault();$('#products .item').addClass('list-group-item');$('#products .item .card').css('width','100%');$('#products .item .card').css('flex-direction','row');});
        $('#gridview').click(function(event){event.preventDefault();$('#products .item').removeClass('list-group-item');$('#products .item').addClass('grid-group-item');$('#products .item .card').css('width','14rem');$('#products .item .card').css('flex-direction','column');});
    });
    ...
    $(".f-card-title").html(function(index, currentText) {
...
<script src="{{ config('app.ui_asset_url').'frontend/js/homePageListing.js' }}"></script>
