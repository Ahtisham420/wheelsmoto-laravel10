...
<ol class="breadcrumb">
    ...
    <li class="breadcrumb-item">
        <a href="">{{ __('admin/pages/roles_from.bread_crumb_2_2') }}</a>
    </li>
</ol>
<!-- Breadcrumb-->
        ...
        <div class="col-md-12">
            <div class="card">
                <form method="post" action="{{route("roles.update",$role->id)}}">
                    @csrf
                    @method('PUT')
                    <input type="hidden" value="{{ !empty($role) ? $role->id : 0 }}" name="id">
                    <div class="card-header">
                        <strong>{{ __('admin/pages/roles_from.card_title_2') }} </strong>
                        {{--<small>Form</small>--}}
                    </div>
                    <div class="row">
                        <div class="col-sm-12">
                                    ...
                                    <div class="card-body">
                                        <div class="form-group">
                                            <div class="row">
                                                ...
                                                <div class="col-md-4">
                                                    <input name="name" type="text" value="{{ !empty($role) ? $role->name : '' }}" class="form-control" required />
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                <div class="col-md-4">
                                                    <select class="select2 form-control {{ $errors->has('attach_permissions') ? 'is-invalid' : '' }}" placeholder="Choose Permissions" name="attach_permissions[]" id="attach_permission" multiple="multiple">
                                                        ...
                                                        @foreach($permissions as $permission)
                                                            <option value="{{ $permission->id }}" {{ (!empty($role) ? ( in_array($permission->id, $role->getPermissionsIds()->toArray() )) ? "selected" : "" : "" )}}>{{ $permission->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
                    ...
                    <div class="card-footer">
                        ...
                        <a class="btn btn-sm btn-danger" type="reset" href="{{route('roles.update',['id' => $role->id])}}">
