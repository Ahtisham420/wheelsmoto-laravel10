<html>
<head>
    <title>Complete Jobs Report</title>
    <style>
    table {
        border-spacing: 0;
...
<h2 class="pdf-text">Jobs Complete Report<br><span class="pdf-small">From {{  $from }} to {{ $to }}</span></h2>
...
<table width="100%">
    ...
    @foreach ($jobs as $job)
    @php
    $total_amount += $job->service_price;
    $total_commision += ($job['service']->commision / 100) * $job->service_price;
    @endphp
        <tr>
            <td>{{ $job->id }}</td>
            ...
            <td>{{ !empty($job['service']) ? ($job['service']->commision / 100) * $job->service_price : "-Na-"  }}</td>
